...
auto Text=Paragraph{};
auto Word_To_Search=Text.contains(Word{"University"});
CHECK_FALSE(get<0>(Word_To_Search));
}
//
TEST_CASE("Word not present in Paragraph cannot be found") {
  auto line="I love software"s;
  auto Text=Paragraph{};
  Text.addLine(line);
  auto Word_To_Search=Text.contains(Word{"University"});
  CHECK_FALSE(get<0>(Word_To_Search));
...
TEST_CASE("Line number of a Word appearing once in Paragraph is returned") {
  auto Text=Paragraph{};
  Line First_Line("I have no choice but to pull through");
  Line Second_Line("All i have to do is to put effort");
  Text.addLine(First_Line);
  Text.addLine(Second_Line);
  auto Word_To_Search=Text.contains(Word{"put"});
  vector<int>List_Of_Lines={2};
  CHECK(get<1>(Word_To_Search)==(List_Of_Lines));
}
//
TEST_CASE("Line numbers of a Word appearing in multiple Lines of a Paragraph is returned") {
    auto Text=Paragraph{};
    Line First_Line("I love hip hop");
    Line Second_Line("Wihtout love and hip hop id be depressed");
    Line Third_Line("You have to love yourself first");
    Text.addLine(First_Line);
    Text.addLine(Second_Line);
    Text.addLine(Third_Line);
    auto Word_To_Search=Text.contains(Word{"love"});
    vector<int>List_Of_Lines={1,2,3};
    CHECK(get<1>(Word_To_Search)==(List_Of_Lines));
}
//
